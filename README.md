...
- First-load bug fixed: the page now opens on Knockout stage only rather than preserving the old All stages default.

- Penalty winner box added for knockout draws:
  - Actual score remains the score after extra time, e.g. `1-1`.
  - Penalties box activates only when the actual score is a draw.
  - `W` means the left/home team wins on penalties.
  - `L` means the right/away team wins on penalties.
  - Penalty data is stored separately in `data.json` under `penalties`, preserving all historical scores.
